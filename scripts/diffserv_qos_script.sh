...
s1r2_URL="http://localhost:8080/v1.0/conf/switches/0000000000000021/ovsdb_addr"
s1r3_URL="http://localhost:8080/v1.0/conf/switches/0000000000000031/ovsdb_addr"
...
echo $'\n' $?
curl -X PUT -d ' "tcp:127.0.0.1:6632" ' "$s1r2_URL"
echo $'\n' $?
curl -X PUT -d ' "tcp:127.0.0.1:6632" ' "$s1r3_URL"
echo $'\n' $?
...
s1r2_queue_URL="http://localhost:8080/qos/queue/0000000000000021"
s1r3_queue_URL="http://localhost:8080/qos/queue/0000000000000031"
s1r4_queue_URL="http://localhost:8080/qos/queue/0000000000000041"

echo "setting the queues params for core router (s1)..." $'\n'
for i in 1 2 3 4; do
  curl -X POST -d '{"port_name": "s1-eth'"$i"'", "type": "linux-htb", "max_rate": "1000000", "queues":[{"max_rate": "1000000"}, {"min_rate": "200000"}, {"min_rate": "500000"}]}' "$s1_queue_URL"
  echo $'\n' $?
done

echo "setting the queues params for edge routers (s1rx)..." $'\n'
...
echo $'\n' $?
curl -X POST -d '{"port_name": "s1r2-eth1", "type": "linux-htb", "max_rate": "1000000", "queues":[{"max_rate": "1000000"}, {"min_rate": "200000"}, {"min_rate": "500000"}]}' "$s1r2_queue_URL"
echo $'\n' $?
curl -X POST -d '{"port_name": "s1r3-eth1", "type": "linux-htb", "max_rate": "1000000", "queues":[{"max_rate": "1000000"}, {"min_rate": "200000"}, {"min_rate": "500000"}]}' "$s1r3_queue_URL"
echo $'\n' $?
